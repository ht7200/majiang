cc.Class({
    ...
    initView: function () {
        ...
        this._gamecount = gameChild
            .getChildByName("gamecount")
            .getComponent(cc.Label);
        this._gamecount.string =
            "" +
            cc.vv.gameNetMgr.numOfGames +
            "/" +
            cc.vv.gameNetMgr.maxNumOfGames +
            "局";
        ...
        for (var i = 0; i < sides.length; ++i) {
            ...
            this._playEfxs.push(
                sideChild.getChildByName("play_efx").getComponent(cc.Animation)
            );
            this._chupaiSprite.push(
                sideChild.getChildByName("ChuPai").children[0].getComponent(cc.Sprite)
            );
        ...
        if (cc.vv.gameNetMgr.conf.type == "xlch") {
            ...
            for (var i = 0; i < hupailist.children.length; ++i) {
                ...
                if (hupainode.active == false) {
                    ...
                    hupainode.getComponent(
                        cc.Sprite
                    ).spriteFrame = cc.vv.mahjongmgr.getSpriteFrameByMJID(pre, pai);
